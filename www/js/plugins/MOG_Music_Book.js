 ...
 *
 * @param 漂浮粒子X方向速度
 * @desc 漂浮粒子X方向的速度。可为小数，可为负数。
 * @default 0
 *
 * @param 漂浮粒子Y方向速度
 * @desc 漂浮粒子Y方向的速度。可为小数，可为负数。
 * @default -1
 *
 */
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_Music_Book');
    ...
    Moghunter.musicbook_command_name = String(Moghunter.parameters['用语-菜单名称'] || "Music Book");
	...
	Moghunter.musicbook_meter_y = Number(Moghunter.parameters['平移-进度条 Y'] || 0);
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "enable_music")  {$gameSystem._music_list[Number(args[1])-1][0] = true;};
	if (command === "disable_music")  {$gameSystem._music_list[Number(args[1])-1][0] = false;};
	return true;
};
