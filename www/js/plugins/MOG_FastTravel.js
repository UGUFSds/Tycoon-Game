 ...
 * 配置分为五个部分，坐标组、地图、类型按钮、选择菜单、杂项数据。
 * 【现已支持插件关联资源的打包、加密】
 * 
 * -----------------------------------------------------------------------------
 * ----关联文件
...
//=============================================================================
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_FastTravel');
    ...
    Moghunter.fastTravel_ListWidth = Number(Moghunter.parameters['选择菜单宽度'] || 200);
	Moghunter.fastTravel_ListHeight = Number(Moghunter.parameters['选择菜单高度'] || 290);
    Moghunter.fastTravel_ListX = Number(Moghunter.parameters['平移-选择菜单 X'] || 616);
	Moghunter.fastTravel_ListY = Number(Moghunter.parameters['平移-选择菜单 Y'] || 145);	
    Moghunter.fastTravel_ListSlideX = Number(Moghunter.parameters['选择菜单起点 X'] || 100);
	Moghunter.fastTravel_ListSlideY = Number(Moghunter.parameters['选择菜单起点 Y'] || 0);	
	Moghunter.fastTravel_ListLayoutX = Number(Moghunter.parameters['平移-选择菜单布局 X'] || 0);
	Moghunter.fastTravel_ListLayoutY = Number(Moghunter.parameters['平移-选择菜单布局 Y'] || -53);
	...
	Moghunter.fastTravel_map_point_max = 20;
