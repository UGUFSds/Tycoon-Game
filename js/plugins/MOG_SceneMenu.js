 ...
 * @param 布局窗口间距
 * @parent 是否手动分配布局位置
 * @desc 多个角色之间的窗口间距。如果为负数，则角色反序。
 * @default 200
 *
 * @param 布局窗口W型间距
 * @parent 是否手动分配布局位置
 * @desc 偶数个角色窗口的垂直方向间距。单位像素。
 * @default 0
 *
 * @param 偏移-角色窗口 X
 * @parent ---角色窗口---
 ...
 *
 * 资源-主菜单布局（menus_main文件夹）
 * 资源-金钱数值  （menus_main文件夹）
 * 资源-魔法圈    （menus_main文件夹）
 *
 * 资源-角色窗口  （menus_main文件夹）
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneMenu');
	Moghunter.scMenu_layoutStatusX = Number(Moghunter.parameters['偏移-角色窗口 X'] || 0);
	Moghunter.scMenu_layoutStatusY = Number(Moghunter.parameters['偏移-角色窗口 Y'] || 0);  
    Moghunter.scMenu_CharX = Number(Moghunter.parameters['偏移-角色前视图 X'] || 20);
	Moghunter.scMenu_CharY = Number(Moghunter.parameters['偏移-角色前视图 Y'] || 0);
	...
	Moghunter.scMenu_Com_space = Number(Moghunter.parameters['按钮间距'] || 48);
	Moghunter.scMenu_Com_w_space = Number(Moghunter.parameters['按钮W型间距'] || 36);	
	...
	Moghunter.scMenu_FaceSelX = Number(Moghunter.parameters['平移-可选角色头像 X'] || 240);
	Moghunter.scMenu_FaceSelY = Number(Moghunter.parameters['平移-可选角色头像 Y'] || 128);
	...
	Moghunter.scMenu_pos_lay_model = String(Moghunter.parameters['布局模式'] || "true");
	Moghunter.scMenu_pos_fix = String(Moghunter.parameters['是否手动分配布局位置'] || "false");	
    Moghunter.scMenu_pos_X = Number(Moghunter.parameters['平移-布局 X'] || 0);
    Moghunter.scMenu_pos_Y = Number(Moghunter.parameters['平移-布局 Y'] || 0);
    Moghunter.scMenu_pos_fix_space = Number(Moghunter.parameters['布局窗口间距'] || 100);
    Moghunter.scMenu_pos_fix_w_space = Number(Moghunter.parameters['布局窗口W型间距'] || 0);
    Moghunter.scMenu_pos_fix_char_h = 1;
    Moghunter.scMenu_pos_fix_status_h = 1;
	
	Moghunter.src_main_Layout = String(Moghunter.parameters['资源-主菜单布局']);
	Moghunter.src_main_MagicCircle = String(Moghunter.parameters['资源-魔法圈']);
	Moghunter.src_main_LayoutStatus = String(Moghunter.parameters['资源-角色窗口']);
	...
	for (Moghunter.i = 1; Moghunter.i <= Moghunter.menu_btn_list_length ; ++Moghunter.i) {
	  Moghunter.line = "String(Moghunter.parameters['自定义关键字-" + Moghunter.i + "'] )";
	  Moghunter.menu_btn_key_list[Moghunter.i] = eval(Moghunter.line);
	  Moghunter.line = "String(Moghunter.parameters['自定义按钮-" + Moghunter.i + "'] )";
	  Moghunter.menu_btn_list[Moghunter.i] = eval(Moghunter.line);
	};
	
	Moghunter.menu_btn_face_list_length = 60;
	Moghunter.menu_btn_face_list = {};
	for (Moghunter.i = 1; Moghunter.i <= Moghunter.menu_btn_face_list_length ; ++Moghunter.i) {
	  Moghunter.line = "String(Moghunter.parameters['角色头像-" + Moghunter.i + "'] )";
	  Moghunter.menu_btn_face_list[Moghunter.i] = eval(Moghunter.line);
	};
	
	Moghunter.menu_act_list_length = 60;
	Moghunter.menu_act_list = {};
	for (Moghunter.i = 1; Moghunter.i <= Moghunter.menu_act_list_length ; ++Moghunter.i) {
	  Moghunter.line = "String(Moghunter.parameters['角色前视图-" + Moghunter.i + "'] )";
	  Moghunter.menu_act_list[Moghunter.i] = eval(Moghunter.line);
	};
...
Scene_Menu.prototype.createSelection = function() {
	...
	for (var i = 0; i < $gameParty.members().length; i++) {
		 ...
		 this._selectionPos[i] = [Moghunter.scMenu_FaceSelX ,Moghunter.scMenu_FaceSelY+ ((4 + this._facesBitmaps[i].height) * i)];
		 }
...
MBustMenu.prototype.posY = function() {
    ...
    return 106 + (space / 2) + (space * this._index);
};
...
MBustMenu.prototype.createCharaters = function() {
	 ...
	 if( Moghunter.scMenu_pos_lay_model === "true" ){
		 if( Moghunter.scMenu_pos_fix === "true" ){
			this._char.x = Moghunter.scMenu_pos_X + this._index * Moghunter.scMenu_pos_fix_space +90 ;
		 }else{
			this._char.x = this.posX() ;	//水平排布+自适应 x
		 }
		 if( Moghunter.scMenu_pos_fix === "true" ){
			this._char.y = Moghunter.scMenu_pos_Y +221 ;
			this._char.y += Moghunter.scMenu_pos_fix_char_h * Moghunter.scMenu_pos_fix_w_space ;
		 }else{
			this._char.y = Graphics.boxHeight/2 + 130 ; //水平排布+自适应 y
		 }
	 }else{
		 if( Moghunter.scMenu_pos_fix === "true" ){
			this._char.x = Moghunter.scMenu_pos_X + 90 ;
			this._char.x += Moghunter.scMenu_pos_fix_char_h * Moghunter.scMenu_pos_fix_w_space ;
		 }else{
			this._char.x = Graphics.boxWidth/2 ;//垂直排布+自适应 x
		 }
		 if( Moghunter.scMenu_pos_fix === "true" ){
			this._char.y = Moghunter.scMenu_pos_Y + this._index * Moghunter.scMenu_pos_fix_space + 220 ;
		 }else{
...
MCharStatus.prototype.createLayoutStatus = function() {
	 ...
	 if( Moghunter.scMenu_pos_lay_model ==="true" ){
		 if( Moghunter.scMenu_pos_fix ==="true" ){
			...
			this._layout.x = this.posX() - 70 ;
		 }
		 if( Moghunter.scMenu_pos_fix ==="true" ){
			this._layout.y = Moghunter.scMenu_pos_Y + 28 ;
			this._layout.y += Moghunter.scMenu_pos_fix_status_h * Moghunter.scMenu_pos_fix_w_space ;
		 }else{
			this._layout.y = Graphics.boxHeight/2 -80 ;
		 }
	 }else{
		 if( Moghunter.scMenu_pos_fix ==="true" ){
			...
			this._layout.x = Graphics.boxWidth /2 - 80 ; 
		 }
		 if( Moghunter.scMenu_pos_fix ==="true" ){
			this._layout.y = Moghunter.scMenu_pos_Y + this._index * Moghunter.scMenu_pos_fix_space + 21 ;
		 }else{
			this._layout.y = this.posY() -110 ;
		 }
	 }
...
Window_MenuStatusM.prototype.drawItemStatus = function(index) {
	...
	this.drawText("",x,y + 32 * 1,64,"left")
	this.drawText("",x,y + 32 * 2,64,"left")
	this.drawText("",x,y + 32 * 3,80)
